...
//      val bestLabel = obj.labels.maxByOrNull { label -> label.confidence } ?: return@mapNotNull null
            ...
            val rotatedBottomRight = Point(obj.boundingBox.right, obj.boundingBox.bottom)
                .rotateCoordinates(rotatedImage.width, rotatedImage.height, imageRotation)
            val rotatedBounds = Rect(rotatedTopLeft.x, rotatedTopLeft.y, rotatedBottomRight.x, rotatedBottomRight.y)

            Log.d("carlos", "(${obj.boundingBox.left}, ${obj.boundingBox.top}) => (${rotatedTopLeft.x}, ${rotatedTopLeft.y})")

            DetectedObjectResult(obj.trackingId!!, 1f, "label", rotatedBounds)
        }
    }
